...
!Specific to VODE    
    ...
    jt  = 25
    lrw = 22 + 11*neq + (3*ml + 2*mu)*neq ! Work variables for dlsoda, max(LRN,LRW), refer to documentation for sizes recommended when jt = 4 or 5, and NEQ is constant
    liw = 30 + neq ! Integer work variable, min value: 20 + neq
...
!-----------------------------------------------------------------------------------
    ...
    call dvode(derivs_vode,neq,yt,time,tout,itol,rtol,atol,itask,istate,iopt,rwork,lrw,iwork,liw,jac_vode,jt,rpar,ipar)
